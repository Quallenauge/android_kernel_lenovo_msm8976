...
#define GIS_FW_IMAGE_NAME "gis_startup_fw_update.img"
...
#define VENDOR_OFILM    0x01
#define VENDOR_OFILM_OLD  0x11

#define VENDOR_GIS      0x06
#define VENDOR_GIS_OLD  0x00
char gis_orig = 1;
...
static ssize_t synaptics_rmi4_fw_id_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_fw_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_vendor_id_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
    ...
    __ATTR(fw_id, S_IRUGO,
			synaptics_rmi4_fw_id_show,
			synaptics_rmi4_fw_id_store),
	__ATTR(vendor_id, S_IRUGO,
			synaptics_rmi4_vendor_id_show,
			synaptics_rmi4_vendor_id_store),
...
static void fwu_compare_partition_tables(void)
{
    ...
    if (gis_orig == VENDOR_GIS_OLD) {   // special treat for gis very first fw.
...
static enum flash_area fwu_go_nogo(void)
{
  ...
  // lxh:modify; let upgrading normal;20160530;
//	if (image_config_id > device_config_id) 
    if (fwu->img.ui_config.data[3] > config_id[3]){
...
static int fwu_start_reflash(void)
{
    ...
    gis_orig = config_id[0];
	if (fwu->image == NULL) {
		// --  for ofilm ---
	 	if (config_id[0] == VENDOR_OFILM || config_id[0] == VENDOR_OFILM_OLD) {
		    retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME, sizeof(FW_IMAGE_NAME),
				sizeof(FW_IMAGE_NAME));
	     }
		// --  for gis ---
        else if (config_id[0] == VENDOR_GIS || config_id[0] == VENDOR_GIS_OLD) {
		    retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				GIS_FW_IMAGE_NAME, sizeof(GIS_FW_IMAGE_NAME),
				sizeof(GIS_FW_IMAGE_NAME));
        } 
 ...
 //	printk(KERN_ERR"lxh:***%d, %s**size = %d\n", __LINE__, __func__, (int)fw_entry->size);
	...
	if (fwu->do_lockdown && (fwu->img.lockdown.data != NULL)) {
		switch (fwu->bl_version) { //  fwu->bl_version is 7...
		case BL_V5:
		case BL_V6:
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
	    ...
	    if (config_id[0] == VENDOR_OFILM)  {
          ret = snprintf(buf, 20, "%s\n", "OFILM");
		} else if(config_id[0] == VENDOR_GIS){  
		  ret = snprintf(buf, 20, "%s\n", "GIS");
        } else
