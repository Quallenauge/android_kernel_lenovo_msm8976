...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	//	msleep(bdata->reset_delay_ms);
   	//   lxh:modify; reset_delay_ms from 100 to 60 will cause 
    //   Partition table mismatch when fwu, so here stay 100ms. ; 20160713;
		 msleep(200);
	}
...
//	msleep(rmi4_data->hw_if->board_data->reset_delay_ms);
    msleep(200);   // lxh:modified; to avoid touch invalid ;20160712; 
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
	//	synaptics_rmi4_sensor_sleep(rmi4_data); // lxh:del; sleep ->poweroff;
		synaptics_rmi4_free_fingers(rmi4_data);
	}
	...
	//	printk(KERN_ERR"lxh:**%s,%d**\n", __func__, __LINE__);
		...
		//	goto err_enable_reg;
		 ...
		 // --set reset low, to avoid current leakage --
         gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
	}   
...
//	synaptics_rmi4_sensor_wake(rmi4_data); // lxh:del;wake -> power on;
	synaptics_rmi4_irq_enable(rmi4_data, true, false);
